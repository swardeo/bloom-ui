...
import validateDebt from '../../util/validateDebt';
import {
    NameField,
    StartAmountField,
    MonthlyAmountField,
...
const DebtsForm = ({ action, debt }) => {
    ...
    const defaultValues = JSON.parse(JSON.stringify(debt));
    ...
    useEffect(() => {
        if (action === 'update') {
            setAdjustmentIndexes(createNumberedArray(debt.adjustments.length));
            setAdjustmentCounter(debt.adjustments.length);
            setOneTimePaymentIndexes(
                createNumberedArray(debt.oneTimePayments.length)
            );
            setOneTimePaymentCounter(debt.oneTimePayments.length);
            setActive(false);
            if (debt?.adjustments?.length > 0) {
                debt.adjustments.sort(
                    (a, b) => new Date(a.dateFrom) - new Date(b.dateFrom)
                );
            }
            if (debt?.oneTimePayments?.length > 0) {
                debt.oneTimePayments.sort(
    ...
    const onSubmit = async (debt) => {
        ...
        if (validation === 'success') {
            ...
            if (action === 'add') {
                ...
                await API.put(config.api.NAME, `/debts/${debt.name}`, {
                    body: debt,
                })
                    .then(() => {
                        setRedirect('/debts');
                ...
                <CenteredGridItem item xs={12}>
                    ...
                    {active && (
                        <Button
                            ...
                            size="large"
                        >
                            {action === 'add' ? 'Add Debt' : 'Update Debt'}
                        </Button>
                    )}
                </CenteredGridItem>
            </Grid>
